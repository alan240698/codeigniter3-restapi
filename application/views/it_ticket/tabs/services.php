<div id="tab-services" class="tab-content active">
    ...
    <div class="setup-progress-container" style="margin-bottom: 30px; padding: 20px; background: linear-gradient(135deg, #0b1c68ff 0%, #18032dff 100%); border-radius: 12px; color: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        ...
        <div class="progress-steps" style="display: flex; gap: 15px;">
            <div id="step-service-groups" class="progress-step" style="flex: 1; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 8px; text-align: center; transition: all 0.3s ease;">
                ...
                <div class="step-title" style="font-weight: bold; margin-bottom: 5px; font-size: 14px;">1. Service Groups</div>
            ...
            <div id="step-ticket-types" class="progress-step" style="flex: 1; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 8px; text-align: center; opacity: 0.5; transition: all 0.3s ease;">
                ...
                <div class="step-title" style="font-weight: bold; margin-bottom: 5px; font-size: 14px;">2. Ticket Types</div>
                <div class="step-count" style="font-size: 20px; font-weight: bold; margin: 5px 0;">0</div>
            ...
            <div id="step-issue-types" class="progress-step" style="flex: 1; padding: 15px; background: rgba(255,255,255,0.1); border-radius: 8px; text-align: center; opacity: 0.5; transition: all 0.3s ease;">
                ...
                <div class="step-title" style="font-weight: bold; margin-bottom: 5px; font-size: 14px;">3. Issue Types</div>
                <div class="step-count" style="font-size: 20px; font-weight: bold; margin: 5px 0;">0</div>
                <div class="step-status" style="font-size: 12px; opacity: 0.8;">🔒 Locked</div>
            </div>
        </div>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div>
                <button id="btnAddIssueType" class="btn btn-primary" disabled
...
<div id="modalServiceGroup" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-primary" onclick="ServiceManager.saveServiceGroup()">
...
<div id="modalTicketType" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-primary" onclick="TicketTypeManager.saveTicketType()">
                <i class="fas fa-save"></i> Save
            </button>
        </div>
    </div>
...
<div id="modalIssueType" class="modal">
    <div class="modal-content">
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-primary" onclick="IssueTypeManager.saveIssueType()">
                <i class="fas fa-save"></i> Save
            </button>
        </div>
    </div>
    ...
    const ServiceManager = {
        ...
        renderServiceGroups(serviceGroups) {
            ...
            <tr>
                ...
                <td><span style="font-size: 24px;">${sg.icon || '📁'}</span></td>
                ...
                <td>
                    <div class="action-buttons">
                        <button class="btn btn-sm btn-primary" onclick="ServiceManager.editServiceGroup(${sg.id})">
                            <i class="fas fa-edit"></i> Edit
                        </button>
                        <button class="btn btn-sm btn-danger" onclick="ServiceManager.deleteServiceGroup(${sg.id})">
        ...
        async editServiceGroup(id) {
            try {
                ...
                ITTicketUI.showError('Error loading service group');
            }
        },
        ...
        async saveServiceGroup() {
            ...
            try {
                ...
                ITTicketUI.showError('Error saving service group');
            }
        },

        async deleteServiceGroup(id) {
            const confirmed = await ITTicketUI.confirm('Confirm Action', 'Are you sure you want to delete this service group?', 'Yes, proceed'); if (!confirmed) return;
    ...
    const TicketTypeManager = {
        ...
        renderTicketTypes(ticketTypes) {
            ...
            <tr>
                ...
                <td>
                    <div class="action-buttons">
                        <button class="btn btn-sm btn-primary" onclick="TicketTypeManager.editTicketType(${tt.id})">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn btn-sm btn-danger" onclick="TicketTypeManager.deleteTicketType(${tt.id})">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </td>
        ...
        async editTicketType(id) {
            try {
                ...
                ITTicketUI.showError('Error loading ticket type');
            }
        },
        ...
        async saveTicketType() {
            ...
            if (!formData.service_group_id || !formData.name || !formData.code) {
                ITTicketUI.showValidationError('Please fill in required fields');
                return;
            }
            ...
            try {
                ...
                ITTicketUI.showError('Error saving ticket type');
            }
        },

        async deleteTicketType(id) {
            const confirmed = await ITTicketUI.confirm('Confirm Action', 'Are you sure you want to delete this ticket type?', 'Yes, proceed'); if (!confirmed) return;
    ...
    const IssueTypeManager = {
        ...
        renderIssueTypes(issueTypes) {
            ...
            container.innerHTML = parents.map(parent => {
                ...
                return `
                <div style="padding: 15px; background: #f9fafb; border-left: 4px solid #3b82f6; margin-bottom: 10px; border-radius: 8px;">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div class="action-buttons">
                            <button class="btn btn-sm btn-primary" onclick="IssueTypeManager.editIssueType(${parent.id})">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-success" onclick="IssueTypeManager.openAddSubIssue(${parent.id})">
                                <i class="fas fa-plus"></i> Sub-Issue
                            </button>
                            <button class="btn btn-sm btn-danger" onclick="IssueTypeManager.deleteIssueType(${parent.id})">
                                <i class="fas fa-trash"></i>
                            </button>
                        </div>
                    </div>
                    ...
                    ${parentChildren.map(child => `
                        <div class="tree-item level-2">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <div class="action-buttons">
                                    <button class="btn btn-sm btn-primary" onclick="IssueTypeManager.editIssueType(${child.id})">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="btn btn-sm btn-danger" onclick="IssueTypeManager.deleteIssueType(${child.id})">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>
                            </div>
        ...
        async editIssueType(id) {
            try {
                ...
                ITTicketUI.showError('Error loading issue type');
            }
        },
        ...
        async saveIssueType() {
            ...
            if (!formData.ticket_type_id || !formData.name || !formData.code) {
                ITTicketUI.showValidationError('Please fill in required fields');
                return;
            }
            ...
            try {
                ...
                ITTicketUI.showError('Error saving issue type');
            }
        },

        async deleteIssueType(id) {
            const confirmed = await ITTicketUI.confirm('Confirm Action', 'Are you sure you want to delete this issue type?', 'Yes, proceed'); if (!confirmed) return;
